...
$stmt = $mysqli->prepare('SELECT * FROM users WHERE users.email=? AND users.birthMonth=? AND users.birthYear=?') or die($mysqli->error);
$stmt->bind_param("sii", $email,$bMonth,$bYear);
$stmt->execute();
